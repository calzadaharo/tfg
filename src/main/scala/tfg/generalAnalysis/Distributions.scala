...
object Distributions{
  def apply() = new Distributions(Array())
}
...
class Distributions(args:Array[String]) extends Analyser(args){
  ...
  override def returnResults(): Any = {
    ...
    val weightDist = view.getVertices().filter(
        x => x.getPropertyValue("original").getOrElse("") == "false"
      ).map{
  ...
  override def processResults(results: ArrayBuffer[Any], timeStamp: Long, viewCompleteTime: Long): Unit = {
    var output_file = System.getenv().getOrDefault("OUTPUT_FILE","Distributions.json").trim
