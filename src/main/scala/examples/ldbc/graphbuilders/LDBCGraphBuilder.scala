...
class LDBCGraphBuilder extends GraphBuilder[String] {
  override def parseTuple(tuple: String) = {
    ...
    fileLine(0) match {
      case "person" =>
        addVertex(creationDate, assignID("person" + fileLine(3)), Type("person"))
        //sendGraphUpdate(VertexAdd(creationDate, fileLine(3).toLong,Type("person")))
        if(vertexDeletion)
          addVertex(deletionDate, assignID("person" + fileLine(3)))
      case "person_knows_person" =>
        ...
        if(edgeDeletion)
          deleteEdge(deletionDate, assignID("person"+fileLine(3)),assignID("person"+fileLine(4)))
    }
  }
}
